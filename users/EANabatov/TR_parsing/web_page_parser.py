...
class BookStoreParser:
    ...
    def __parsing(self, link=str) -> dict:
        ...
        try:
            ...
            if self.__page.getcode() != 200:
                ...
                raise AssertionError(f"Error code: {self.__page.getcode()}")
            if not self.__soup.find(
                href=re.compile("__books"), class_="link_for_parsing-gray-light"
            ):
    ...
    def __book_name(self):
        """Название книги"""
        return (
            self.__soup.find("span", class_="link_for_parsing-gray-light")
            .text.replace(" ", " ")
            .strip()
        )
    ...
    def __is_done(self) -> None:
        self.__status = False

    def __return_data_from_parsing(self) -> List[dict]:
        return self.__books
    ...
    def start_parsing(self, start_parsing_code, end_parsing_code) -> None:
        ...
        self.__return_data_from_parsing()
        self.__is_done()
