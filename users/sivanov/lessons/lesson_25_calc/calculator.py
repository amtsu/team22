...
=======
        ...
        if (isinstance(value.body.left,ast.UnaryOp)):
            ...
            self.__left = value.body.left.value
        if (isinstance(value.body.right,ast.UnaryOp)):
            assert(isinstance(value.body.right.operand, ast.Constant)) # проверяем корректность правого аргумента
            self.__right = self.un_ops[type(value.body.right.op)](value.body.right.operand.value)
        else:
            self.__right = value.body.right.value
        return self.__bin_ops[type(value.body.op)](self.__left, self.__right) # пытаемся вычислить результат
