...
class MyListData:
    ...
    def __init__(self):
        self.__TODO = [u'купить хлеб',u'помыть полы', u'дернуть']
        self.__TIMETOBEDONE = [u'28-2-2022',u'28-2-2022',u'28-2-2022']
        self.__TIMEDONE = [u'28-2-2022',u' ',u'1-3-2022']
        self.fill_this_shit()
    ...
    def add_action(self,whattodo,whenitshoudbedone,whenitsdone = u''):
    ...
    def print_lists(self):
        ...
        for i in range(0,count):
            print(unicode(i) + u': ' + unicode(self.__TIMETOBEDONE[i]) + u' собирался '+unicode(self.__TODO[i])+u', выполнил '+  unicode(self.__TIMEDONE[i]))
        print('---------------------------------------------------')
        return None
    ...
    def fill_this_shit(self):
        i=1
        while i < 7:
            self.add_action(u'промыть полы', self.make_date(i*2,3,2022))
            i=i+1
        i=1
        while i < 7:
            self.add_action(u'купить хлеб', self.make_date(i*3,3,2022))
            i=i+1
        i=1
        while i < 8:
            self.add_action(u'дернуть', self.make_date(i*4,3,2022))
            i=i+1
        return None 
    ...
    def save_to_json(self, filename):
        ...
        data = []
        for i in range(0,len(self.__TODO)):
            data.append([self.__TODO[i],self.__TIMETOBEDONE[i],self.__TIMEDONE[i]])
        rez = json.dumps(data)
    ...
    def load_from_json(self, filename):
        ...
        for i in range(0,len(indata)):
            self.__TODO.append(indata[i][0]) 
            self.__TIMETOBEDONE.append(indata[i][1])
            self.__TIMEDONE.append(indata[i][2])        
        return indata
    #очистить списки
    def clear(self):
        self.__TODO = []
